...
public:
    ...
    void filter_by_exact_height(double exactHeight) const {
        vector<PressureDataItem> filteredData;

        for (const auto& item : pressureData) {
            if (item.height == exactHeight) {
                filteredData.push_back(item);
            }
        }

        if (!filteredData.empty()) {
            sort(filteredData.begin(), filteredData.end(), [](const PressureDataItem& a, const PressureDataItem& b) {
            ...
            cout << "Нет данных для указанной высоты." << endl;
        }
    }
};
